...
- **フレームレス表示** — タイトルバー・枠なし。ウィンドウ操作ボタン（macOS は左上に「閉じる / 最小化 / 最大化」、Windows / Linux は右上に「最小化 / 最大化 / 閉じる」）や左右の移動ボタン、左下のファイル名・右下の枚数表示は、マウスを動かしている間だけオーバーレイ表示され、3 秒動かさないと消えて画像だけの表示に戻ります。最小化・最大化の挙動は OS 標準どおりで、タイトルバーのダブルクリックでも最大化できます。大きさはウィンドウの端をドラッグしても変えられます
...
- **画像に合わせる** — 余白が出ないよう、画像の縦横比ぴったりにウィンドウの大きさを合わせます。前後の画像へ移動するたびに合わせ直します。画面に収まらない大きな画像は、画面（タスクバーなどを除いた作業領域）の 90% に収まるよう縮小し、ウィンドウが画面からはみ出す場合は中へ寄せます。画像を開いていないとき（アプリを単体で起動した直後など）は従来どおり既定の大きさで開きます

ウィンドウの位置は、どちらの設定でも前回閉じたときの場所を覚えていて、次回もそこで開きます（画面からはみ出す場合は中へ寄せます。前回使っていたディスプレイが外れているなど、保存した位置がどの画面にも載らない場合は既定どおり中央に開きます）。

固定用のウィンドウサイズとウィンドウ位置は、設定とは別の `window.json` に保存します（`settings.json` と同じ `sview` フォルダに置きますが、設定の「既定に戻す」では消えません）。最小化中・最大化中に閉じた場合は、その状態ではなく元のサイズと位置を覚えます。

「画像に合わせる」で最大化している間は、画像を切り替えてもウィンドウの大きさを変えません（変えると最大化が解けてしまうため）。元のサイズに戻すと、また画像ごとに合わせるようになります。
...
| 分類 | 項目 |
| --- | --- |
| 外観 | 背景色 / 背景の不透明度 / ウィンドウの角丸 / ファイル名（左下）と枚数（右下）の表示 / 左右の移動ボタン表示 |
