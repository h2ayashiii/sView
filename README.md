...
- **画像に合わせる** — 余白が出ないよう、ウィンドウの形を表示中の画像の縦横比に合わせます。前後の画像へ移動するたびに合わせ直し、手でサイズを変えたときも縦横比を保ちます（ドラッグしている最中ではなく、手を離して落ち着いたところで合わせます。動かした幅が大きい方の辺をそのまま活かし、もう一方を縦横比から決めます）。ウィンドウの広さ（面積）は引き継ぐので、縦長と横長の画像を行き来しても大きさの印象は変わりません。画面（タスクバーなどを除いた作業領域）の 90% に収まらない場合は、縦横比を保ったまま縮めます
