...
- **画像に合わせる** — 余白が出ないよう、ウィンドウの形を表示中の画像の縦横比に合わせます。前後の画像へ移動するたびに合わせ直し、ウィンドウの端や角をドラッグして大きさを変えるときも、引っ張った辺に合わせて縦横比を保ったまま変わります（縦横比から外れた形にはできません）。ウィンドウの広さ（面積）は引き継ぐので、縦長と横長の画像を行き来しても大きさの印象は変わりません。大きさは画面（タスクバーなどを除いた作業領域）の 90% までに収めます（それ以上大きくしたいときは最大化か全画面表示を使ってください）
